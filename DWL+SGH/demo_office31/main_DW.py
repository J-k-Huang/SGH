...
# Training settings
parser = argparse.ArgumentParser(description='Office31 Classification')
...
parser.add_argument('--name', type=str, default='board', metavar='B',
                    help='board dir')
parser.add_argument('--save', type=str, default='/media/zrway/8T/HJK/datasets/office31/save_model/', metavar='B',
                    help='board dir')
parser.add_argument('--train_path', type=str, default='/media/zrway/8T/HJK/datasets/office31/dslr/images', metavar='B',
                    help='directory of source datasets')
parser.add_argument('--val_path', type=str, default='/media/zrway/8T/HJK/datasets/office31/webcam/images', metavar='B',
...
parser.add_argument('--task_name', type=str, default='amazon-webcam', metavar='B',
                    help='domain1-domain2')
os.environ["CUDA_VISIBLE_DEVICES"] = '0'
#amazon-webcam-dslr
#-----------
#parser.add_argument('--save', type=str, default='/media/zrway/8T/HJK/demo_ImageCLEF/save_model/', metavar='B',
#                   help='board dir')

#parser.add_argument('--train_path', type=str, default='/media/zrway/8T/HJK/demo_ImageCLEF/image_clef/i', metavar='B',
#                    help='directory of source datasets')
#parser.add_argument('--val_path', type=str, default='/media/zrway/8T/HJK/demo_ImageCLEF/image_clef/p', metavar='B',
#                    help='directory of target datasets')
#parser.add_argument('--resnet', type=str, default='50', metavar='B',help='which resnet 18,50,101,152,200')
#parser.add_argument('--task_name', type=str, default='c-p', metavar='B',help='domain1-domain2')
#-------
#i-p-c
'''
parser.add_argument('--train_path', type=str, default='/home/ps/xiaoni/datasets/OfficeHome/Product', metavar='B',
                    help='directory of source datasets')
parser.add_argument('--val_path', type=str, default='/home/ps/xiaoni/datasets/OfficeHome/Clipart', metavar='B',
                    help='directory of target datasets')
parser.add_argument('--resnet', type=str, default='50', metavar='B',
                    help='which resnet 18,50,101,152,200')
parser.add_argument('--task_name', type=str, default='Product-Clipart', metavar='B',
                    help='domain1-domain2')
#Art-Product-Clipart-RealWorld /home/ps/xiaoni/datasets/OfficeHome/Art
'''
...
def ent(output):
    #output_background =  torch.DoubleTensor([1e-6,1e-6,1e-6,1e-6,1e-6,1e-6,1e-6,1e-6,1e-6,1e-6,1])
    #output_onehot = self.props_to_onehot(output)
    #out = -torch.mean(F.softmax(output_background.cuda()) * torch.log(F.softmax(output_onehot.cuda() + 1e-6)))
    #out = -torch.mean(torch.log(F.softmax(output.cuda() + 1e-6)))
...
def train(num_epoch):
    ...
    for ep in range(1,num_epoch):
        ...
        D.train()
        #if ep == 14:
             #G.load_state_dict(torch.load("save_model_4_101_tensor(62)_G.pth")) 
             #F1.load_state_dict(torch.load("save_model_4_101_tensor(62)_F1.pth"))
             #F2.load_state_dict(torch.load("save_model_4_101_tensor(62)_F2.pth")) 
        
        #----------XYY ADD FOR J(w)------#
        #feat_s1 = np.zeros(shape=(498,2048)) #A:2817 D:498
        #feat_t1 = np.zeros(shape=(2817,2048))
        #feat_s = np.zeros(shape=(dset_s_sizes,2048)) 
        #feat_t = np.zeros(shape=(dset_t_sizes,2048))
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            data1 = data['S']
            target1 = data['S_label']
            if data1.size()[0] < batch_size or data2.size()[0] < batch_size:
                break
            data1 = data1.cuda()
            data2 = data2.cuda()
            imgs = Variable(torch.cat((data1, \
                                       data2), 0))
...
################### source domain is discriminative.###################
            for i1 in range(num_k1):
                ...
                #print("output_cr_s_C:",output_cr_s_C)
                #print("target1:",target1)
...
##################### discriminablity ##################
                ...
                #loss_dis2_t = self.discrepancy(output_cr_t_C, output_cr_t_C1)
                #loss_dis3_t = self.discrepancy(output_cr_t_C, output_cr_t_C2)
                loss_3 = loss_cr_s + loss_dis1_t #+ loss_dis2_t + loss_dis3_t
                loss_3.backward()
                opt_c1c2.step()
                opt_c.step()
                reset_grad()
...
################ the balance of transferability and discriminability ###########
  ...
  #----------------grad balance--------------------------------------------
                ...
                #g=[]
                #g.append(torch.tensor(param.grad).view(-1).contiguous())
                #grad1=torch.cat(g,dim=0).detach()
                grad1=getgrad(G)
                opt_g.zero_grad()
                loss_4.backward(retain_graph=True)
                #g=[]
                #g.append(torch.tensor(param.grad).view(-1).contiguous())
                #grad2=torch.cat(g,dim=0).detach()
                grad2=getgrad(G)
                opt_g.zero_grad()
                ...
                #loss_all = T * loss_4 + (1.0 - T) * loss_5 -GA * T* loss_4 -GB *(1.0 - T)* loss_5 #(2)
                loss_balance = T*loss_1 + (1.0-T)*loss_4-GA * T* loss_1 -GB *(1.0 - T)* loss_4 #+ offset*(loss_1 + loss_4)
                #loss_balance = T*loss_1 + (1.0-T)*loss_4
...
############ re-weiting based on the uncertainity of pseudo-label ###############
            ...
            #output_cr_s_D = D(feat_cr_s.cuda())
            #output_cr_t_D = D(feat_cr_t.cuda())
            #print("output_di_s_Dhhhhhhhhhhhhhhhhhhhhhhh")
            #print(output_di_s_D)
            #A_cr = A_cr + output_cr_s_D[output_cr_s_D.reshape(-1) < 0.5].shape[0] 
            #A_cr = A_cr + output_cr_t_D[output_cr_t_D.reshape(-1) >= 0.5].shape[0]          
            #A_all1 = A_all1 + output_cr_s_D[output_cr_s_D.reshape(-1) <= 1.0].shape[0] 
            #A_all2 = A_all2 + output_cr_t_D[output_cr_t_D.reshape(-1) <= 1.0].shape[0]
            #================== A-distance end =====================#

            #====================data for A_st and J(w) begin==============#
            feat_s = G(data1)
            feat_t = G(data2)
            #label_s
            ...
            '''
            #------predict taeget label------#
            label_t_test = target2
            label_t_test_np = label_t_test.cpu().detach().numpy() 
            label_t_for_tSNE = np.concatenate((label_t_for_tSNE,label_t_test_np),axis=0)
            '''
            ...
            if batch_idx % args.log_interval == 0:
                print('Train Ep: {} [{}/{} ({:.0f}%)]\tLoss1: {:.6f}\tLoss2: {:.6f}\tLoss3: {:.6f}\tLoss4: {:.6f}\tLoss5: {:.6f}'.format(
                    ep, batch_idx * len(data), 70000,
                    100. * batch_idx / 10000, loss_1.item(),loss_2.item(), loss_3.item(), loss_4.item(), loss_4.item()))
        ...
        A_st_min = min(abs(A_st_min),abs(A_st))
        A_st_norm = abs(A_st-A_st_min)/(A_st_max-A_st_min+1e-6)
        #print("A_st_norm//A_st_norm//A_st_norm//A_st_norm//A_st_norm//")
        #print(A_st_norm)
        ...
        #print(label_for_LDA)
        #print(fea_for_LDA[label_for_LDA.reshape(-1) ==1].shape)
	...
	#between_class scatter matrix
        ...
        #data_ndim = np.dot(fea_for_LDA, w)

        J_w_s = J_max1/len(clusters1)
        #min_Jw_s_1 = min(min_Jw_s_1,1.0/J_w_s)
	...
	#between_class scatter matrix
        ...
        #max_J_w_1 = max(max_J_w_1,1.0/J_w)
        J_w = min(J_w_s,J_w_t)
        J_w_norm = (J_w -min_J_w)/(max_J_w-min_J_w+1e-6)
        #J_w_1_norm = J_w_1_norm_complex.real
        #print("1/J_W--1/J_W--1/J_W--1/J_W--1/J_W--1/J_W")
        #print(1.0/J_w)
        #print("1/J_W_s--1/J_W_S--1/J_W_S--1/J_W_S--1/J_W_S")
        #print(1.0/J_w_s)
        #print("1-J_W_norm//1-J_W_norm//1-J_W_norm//1-J_W_norm//1-J_W_norm//")
        #print(1.0-J_w_norm)
        ################################################
...
def test(epoch,acc):
    ...
    for batch_idx, data in enumerate(dataset_test):
        ...
        feat = G(data1)
        #feat = feat_1 + feat_2
        #feat_o = torch.cat((feat_o1,0.02*output_so1),1)
        output1 = C(feat)
        output21 = C1(feat)
        output22 = C2(feat)
