...
import argparse
import torch
from solver_grad import Solver
import os
...
parser.add_argument('--record_num', type=str, nargs='?', default='0', help="vision")
...
def main():
    ...
    if args.source == 'usps' or args.target == 'usps':
        record_train = 'record/%s_%s_k_%s_alluse_%s_onestep_%s_%s_grad.txt' % (
            args.source, args.target, args.num_k, args.all_use, args.one_step, record_num)
        record_test = 'record/%s_%s_k_%s_alluse_%s_onestep_%s_%s_test_grad.txt' % (
            args.source, args.target, args.num_k, args.all_use, args.one_step, record_num)
        ...
        record_train = 'record/%s_%s_k_%s_onestep_%s_%s_grad.txt' % (
            args.source, args.target, args.num_k, args.one_step, record_num)
        record_test = 'record/%s_%s_k_%s_onestep_%s_%s_test_grad.txt' % (
            args.source, args.target, args.num_k, args.one_step, record_num)
        while os.path.exists(record_train):
            record_num += 1
            record_train = 'record/%s_%s_k_%s_onestep_%s_%s_grad.txt' % (
