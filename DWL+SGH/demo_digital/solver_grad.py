...
# Training settings
class Solver(object):
    def __init__(self, args, batch_size=128, source='svhn',
...
#################### transferability ########################
            for i2 in range(self.num_k2):
                ...
                #output_cr_t_D = output_cr_t_D.squeeze()
...
################ the balance of transferability and discriminability ###########
            ...
            #----------------grad balance--------------------------------------------
                ...
                a=torch.dot(grad1,grad2)   
                a=a.cpu().numpy()
                np.savetxt('/media/zrway/8T/HJK/demo_digital/checkpoint/'+str(epoch)+str(batch_idx)+'qian.txt',a.reshape(-1, 1))
                self.opt_g.zero_grad()
                
                if torch.dot(grad1,grad2)<0:
                    GA=(torch.dot(grad1,grad2))/(torch.dot(grad1,grad1))
                    GB=(torch.dot(grad1,grad2))/(torch.dot(grad2,grad2)) 
                    #grad_1=grad1-GB*grad2
                    #grad_2=grad2-GA*grad1
                    #m=grad1+grad2
                    #n=grad_1+grad_2
                    #grad1m=grad1.cuda().data.cpu().numpy()
                    #grad2m=grad2.cuda().data.cpu().numpy()
                    #grad_1m=grad_1.cuda().data.cpu().numpy()
                    #grad_2m=grad_2.cuda().data.cpu().numpy()
                    # #m1=m.cuda().data.cpu().numpy()
                   # #n1=n.cuda().data.cpu().numpy()
                    #grad11=np.linalg.norm(grad1m,ord=2)
                    #grad12=np.linalg.norm(grad2m,ord=1)
                    #grad21=np.linalg.norm(grad_1m,ord=1)
                    #grad22=np.linalg.norm(grad_2m,ord=1)
                    ##m2=np.linalg.norm(m1,ord=2)
                    ##n2=np.linalg.norm(n1,ord=2)
                    #print('m:',m)
                    #print('dir1:',m/(m2))
                    #print('n:',n)
                    #print('dir2:',n/(n2))
                ...
                #print('GA:',GA)
                #print('GB:',GB)
                #loss_all11=T * loss_4 + (1.0 - T) * loss_5
                #grad11=grad1.cuda().data.cpu().numpy()
                #m=np.linalg.norm(grad11,ord=2)
                #grad22=grad2.cuda().data.cpu().numpy()
                #n=np.linalg.norm(grad22,ord=2)
                #cos=torch.dot(grad1,grad2)/(m*n)
                #loss_all = T * loss_4 + (1.0 - T) * loss_5 -GA * T* loss_4 -GB *(1.0 - T)* loss_5#- torch.norm(mmm, 'nuc') #(2)
                #loss_all = T * loss_4 + (1.0 - T) * loss_5
                loss_all = (1-GA)*T * loss_4 + (1-GB)*(1.0 - T) * loss_5
                #loss_all = loss_4 +loss_5 -GA * loss_4 -GB *loss_5
                
                #print('loss_all11:',loss_all11)
                #print('loss_all22:',loss_all)
                #print('loss_4:',loss_4)
                #print('loss_5:',loss_5)
                #if loss_all<0:
                ...
                #else:
                    #loss_all=loss_all
               # print('loss_aaa:',loss_aaa)
               # print('loss_all:',loss_all)
                #loss_all = T*loss_4 + (1.0-T)*loss_5+(1.0-GA)*loss_4+(1-GB)*loss_5 #+ self.offset*(loss_4 + loss_5)#(1)
                # loss_all =(1.0 - GA) * loss_4 + (1 - GB) * loss_5#(0)
                # print('loss_4:', loss_4)
                # print('loss_5:', loss_5)
                # print('loss_all:',loss_all)
                loss_all.backward()
...
############ re-weiting based on the uncertainity of pseudo-label ###############
            for i4 in range(self.num_k4):
                ...
                # softmax_out = nn.Softmax(dim=1)(output_cr_t_C)
...
#================ data for A_st and J_w ==============================#
	...
	#between_class scatter matrix
        ...
        print('J_w:',J_w)
        print('J_w_t:',J_w_t)
        print('J_w_norm:',J_w_norm)
        print('min_J_w:',min_J_w)
        print('max_J_w:',max_J_w)
        '''
...
#test--------------test---------------------------
    def test(self, acc_max, epoch, record_file=None, save_model=False):
        ...
        for batch_idx, data in enumerate(self.dataset_test):
            ...
            pred11=pred1.cpu().numpy()
            #np.savetxt('/media/zrway/8T/HJK/demo_digital/checkpoint/'+str(epoch)+'lab.txt',label0)
            #np.savetxt('/media/zrway/8T/HJK/demo_digital/checkpoint/'+str(epoch)+'pre.txt',pred11)
        ...
        if acc > acc_max:
            acc_max = acc
            print(
                'Test Epoch: {} \n Accuracy C: {}/{} ({:.1f}%), Max Accuracy: ({:.1f}%), error: ({:.1f})\n'.format(
                    epoch, correct1, size, acc1, acc,100-acc))
