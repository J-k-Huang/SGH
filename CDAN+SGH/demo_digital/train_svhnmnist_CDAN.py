...
def train(args, model, ad_net, random_layer, train_loader, train_loader1, optimizer, optimizer_ad, epoch, start_epoch, method):
    ...
    for batch_idx in range(num_iter):
        ...
        if epoch > start_epoch:
            ...
            loss_func.CDAN([feature, softmax_output], ad_net, None, None, random_layer).backward(retain_graph=True)
            hook2.remove()
            loss=loss1-GA * loss1 -GB *loss_func.CDAN([feature, softmax_output], ad_net, None, None, random_layer)+loss_func.CDAN([feature, softmax_output], ad_net, None, None, random_layer)
            #elif method == 'DANN':
...
def main():
    ...
    source_list = '/media/zrway/8T/HJK/CDAN-master/data/svhn2mnist/svhn_balanced.txt'
    target_list = '/media/zrway/8T/HJK/CDAN-master/data/svhn2mnist/mnist_train.txt'
    test_list = '/media/zrway/8T/HJK/CDAN-master/data/svhn2mnist/mnist_test.txt'
    ...
    #state = {"model": model, "ad_net": ad_net,"optimizer": optimizer,"optimizer_ad":optimizer_ad,"random_layer":random_layer}
    #torch.save(obj=state, f="/media/zrway/8T/HJK/CDAN-master/save_model/train_svhnmnist.pth")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #state = {"model": model.state_dict(), "ad_net": ad_net.state_dict(),"optimizer": optimizer,"optimizer_ad":optimizer_ad,"random_layer":random_layer,"epoch":epoch}
        #torch.save(obj=state, f='/media/zrway/8T/HJK/CDAN-master/save_model'+str(epoch)+'train_svhnmnist.pth')
